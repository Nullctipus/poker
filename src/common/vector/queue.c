...
#define tail(queue)                                                            \
  wrap(queue->head + queue->base_vector.length, queue->base_vector.capacity)
...
void Queue_Reserve(Queue *queue, size_t size) {
  ...
  if (queue->base_vector.length > 0) {
    if (tail(queue) <= queue->head) {
      memcpy(newData, ((char *)queue->base_vector.pData) + queue->head,
             (queue->base_vector.capacity - queue->head) *
                 queue->base_vector.elementSize);

      memcpy(((char *)newData) + (queue->base_vector.capacity - queue->head) *
                                     queue->base_vector.elementSize,
...
void Queue_Enqueue(Queue *queue, const void *element) {
  ...
  memcpy((char *)queue->base_vector.pData +
             tail(queue) * queue->base_vector.elementSize,
         element, queue->base_vector.elementSize);
...
void *Queue_Peek(Queue *queue, int index) {
  return Vector_At(&queue->base_vector,
                   wrap(queue->head + index, queue->base_vector.capacity));
}
