...
TextBox *create_TextBox(Rectangle backgroundRect, char *title, char *text, int textMaxLength, int draggable)
{
    ...
    //enscripten doesn't support secure functions
...
enum textbox_button update_TextBox(TextBox *textBox)
{
    ...
    if(textBox->hovering){
        ...
        while(key > 0)
        {
            if((key >= 32) && (key <= 125) && (textBox->textLength < textBox->textMaxLength)){
                textBox->text[textBox->textLength] = (char)key;
                textBox->text[textBox->textLength+1] = '\0'; // Add null terminator at the end of the string.
                textBox->textLength++;
            }
            key = GetCharPressed();
        }
        if (IsKeyPressed(KEY_BACKSPACE))
            {
                textBox->textLength--;
                if (textBox->textLength < 0) textBox->textLength = 0;
    ...
    if(update_Button(textBox->submit)) return SUBMIT;
    if(update_Button(textBox->cancel)) return CANCEL;
