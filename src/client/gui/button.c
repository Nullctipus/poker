...
void draw_Button(Button *button) {
  ...
  DrawRectangleRec(
      scaled, (button->hovered ? BUTTON_BACKGROUND_HOVER : BUTTON_BACKGROUND));
#if BUTTON_BORDER != 0
  DrawRectangleLinesEx(scaled, BUTTON_BORDER,
                       (button->hovered ? BUTTON_BACKGROUND_BORDER_HOVER
                                        : BUTTON_BACKGROUND_BORDER));
#endif
  DrawText(button->text, scale * (button->rect.x + button->textOffset),
           scale * (button->rect.y + BUTTON_MARGIN_TOP),
           scale * BUTTON_TEXT_SIZE, BUTTON_TEXT);
}
#pragma warning(pop)