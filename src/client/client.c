...
{
    Game *game = GetGame();
    scanf(value, "%d,%[^,],%llu,%[^,],%llu,%[^,],%llu,%[^,],%llu,%[^,],%llu,%[^,],%llu,%[^,],%llu",
    game->Players.length,
     GetPlayerAt(0)->name, &GetPlayerAt(0)->id,
     GetPlayerAt(1)->name, &GetPlayerAt(1)->id,
     GetPlayerAt(2)->name, &GetPlayerAt(2)->id,
     GetPlayerAt(3)->name, &GetPlayerAt(3)->id,
     GetPlayerAt(4)->name, &GetPlayerAt(4)->id,
     GetPlayerAt(5)->name, &GetPlayerAt(5)->id,
     GetPlayerAt(6)->name, &GetPlayerAt(6)->id);
}
