...
void Vector_Pop(Vector *vector){
  ...
  if(vector->length < vector->capacity/2){
    vector->capacity/=2;
    vector->pData = realloc(vector->pData,vector->capacity*vector->elementSize);
  }
}
...
void Vector_RemoveAt(Vector *vector, unsigned long long index, int bOrdered){
  ...
  if(!bOrdered)
    memmove(Vector_At(vector,index), Vector_At(vector,vector->length-1), vector->elementSize);
  else
    memmove(Vector_At(vector,index),Vector_At(vector,index+1),vector->elementSize * (vector->length-index-1));
  --vector->length;
}
...
void Vector_Reduce(Vector* vector){
  Vector_Reserve(vector,vector->length);
}
...
void Vector_Insert(Vector *vector, unsigned long long index, void *element){
  ...
  Vector_PushBack(vector, element);
  memmove(Vector_At(vector,index+1),Vector_At(vector, index),vector->elementSize*(vector->length-index));
