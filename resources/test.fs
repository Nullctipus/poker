...
void main()
{
    ...
    for(float i = _StartLength; i < _EndLength; i+= _IncLength){
        v += float(length(uv + vec2(
            i * cos(i*_HorizontalWrapCount*sin(time))*i*sin(_HorizontalLoopCount*cos(time*_HorizontalSpeed)*i*cos(i))
            ,
            sin(i * _VerticalWrapCount * sin(time)) * sin(_VerticalLoopCount*sin(time*_VerticalSpeed)*i*i+sin(cos(time)+time))
        )) < _ThicknessMultiplier*(_ThicknessOffset+_ThicknessDelta*sin(i*_ThicknessFreq)));
    }
    
    //return fixed4(Lerp(col.rgb,hueShift(_EffectColor.rgb, frac(_Time.y * _EffectHue)),min(v,1.0)),col.a + min(v,1.0) * (_EffectColor.a - col.a));
    gl_FragColor = vec4(min(v,1.0)*_EffectColor.rgb,_EffectColor.a * min(v,1.0));
